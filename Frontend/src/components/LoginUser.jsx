...
//       navigate("/home-user"); // Redirect to home page after login
//     } catch (err) {
...
export default function LoginUser({setIsLogin}) {
  ...
  const handleEmailSubmit = async (e) => {
    ...
    try {
      const response = await fetch("http://localhost:4000/request-otp", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleOtpSubmit = async (e) => {
    ...
    try {
      ...
      navigate("/home-user"); // Redirect to home page
    } catch (err) {
      setError(err.message);
    } finally {
      setLoading(false);
